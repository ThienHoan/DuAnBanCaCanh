...
public class ProductDetailServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            System.out.println("ProductDetailServlet - Found " + reviews.size() + " reviews for product ID: " + productId);
            ...
            for (Review review : reviews) {
                ...
                System.out.println("  - Found " + images.size() + " images for review ID: " + review.getReviewId());
            ...
            System.out.println("Review statistics - Average: " + averageRating + 
                              ", Total: " + totalReviews + 
                              ", Counts: " + ratingCounts[0] + "," + ratingCounts[1] + "," + 
                              ratingCounts[2] + "," + ratingCounts[3] + "," + ratingCounts[4]);
            
            // Get current user
            model.entity.User currentUser = SessionUtils.getUser(request.getSession());
            
            // Check if user has purchased this product
            boolean canReview = false;
            if (currentUser != null) {
                canReview = reviewService.hasUserPurchasedProduct(currentUser.getUserId(), productId);
                System.out.println("User " + currentUser.getUserId() + " can review product " + productId + ": " + canReview);
            }
            
            // Tính toán đánh giá cho sản phẩm liên quan
//            Map<Integer, Float> relatedProductRatings = new HashMap<>();
//            List<Product> relatedProducts = (List<Product>) request.getAttribute("uniqueNameProductSameCategory");
//            if (relatedProducts != null) {
//                for (Product relatedProduct : relatedProducts) {
//                    float relatedRating = reviewService.getAverageRatingByProductId(relatedProduct.getProductId());
//                    relatedProductRatings.put(relatedProduct.getProductId(), relatedRating);
//                }
//            }
...
//            request.setAttribute("relatedProductRatings", relatedProductRatings);
