...
<html class="no-js" lang="en">
                                    ...
                                    <div class="mobile-search-content">
                                        <form action="#" class="form-search" name="mobile-seacrh" method="get">
                                            ...
                                            <select name="category">
                                                ...
                                                <option value="Ca_Nuoc_Ngot">Cá Nước Ngọt</option>
                                                <option value="Ca_Nuoc_Man">Cá Nước Mặn</option>
                                                <option value="Ca_Nuoc_Lo">Cá Nước lợ</option>
    ...
    <form action="${pageContext.request.contextPath}/category-search" method="get" accept-charset="UTF-8" id="searchForm">
        <input type="text" name="s" class="input-text" placeholder="Search here..." value="${param.s != null ? param.s : ''}" oninput="filterProducts()">
        <select name="category" id="categoryFilter" onchange="filterProducts()">
            <option value="-1" ${param.category == '-1' ? 'selected' : ''}>All Categories</option>
            <option value="Ca_Nuoc_Ngot" ${param.category == 'Ca_Nuoc_Ngot' ? 'selected' : ''}>Cá Nước Ngọt</option>
            <option value="Ca_Nuoc_Man" ${param.category == 'Ca_Nuoc_Man' ? 'selected' : ''}>Cá Nước Mặn</option>
            <option value="Ca_Nuoc_Lo" ${param.category == 'Ca_Nuoc_Lo' ? 'selected' : ''}>Cá Nước lợ</option>
            <option value="Thiet_bi" ${param.category == 'Thiet_bi' ? 'selected' : ''}>Thiết bị</option>
        </select>
        ...
        <c:choose>
            <c:when test="${not empty allProducts}">
                <div class="product-grid" id="productGrid">
                    <c:forEach var="p" items="${allProducts}" varStatus="loop">
                        <div class="product" data-category="${p.categoryId != null ? p.categoryId : '0'}">
                            <img src="${pageContext.request.contextPath}/images/default.jpg" alt="${p.name}">
                            <div class="product-name">${p.name}</div>
                            <div>${p.shortDescription}</div>
                            <div class="price">
                                <c:choose>
                                    <c:when test="${p.salePrice != null && p.salePrice > 0}">
                                        <span class="old-price">${p.price} đ</span><br>
                                        <strong>${p.salePrice} đ</strong>
                                    </c:when>
                                    <c:otherwise>
                            ...
                            <a href="${pageContext.request.contextPath}/add-to-cart?productId=${p.productId}" class="btn">🛒 Thêm vào giỏ</a>
                        </div>
                    </c:forEach>
                </div>
            </c:when>
            <c:otherwise>
                <p style="color:red; font-weight:bold">
                    Không tìm thấy sản phẩm nào với từ khóa: "<c:out value='${searchQuery}'/>"
                </p>
            </c:otherwise>
        </c:choose>
        ...
        <div class="mobile-block-global">
            <div class="biolife-mobile-panels">
                ...
                <a class="biolife-close-btn" data-object="global-panel-opened" href="#">×</a>
            </div>
        ...
        <!-- JavaScript để lọc theo danh mục với ánh xạ chính xác -->
        <script>
function filterProducts() {
    ...
    if (!categorySelect || !searchInput || !productGrid) {
        console.error("Missing elements:", { categorySelect, searchInput, productGrid });
        return;
    }
    ...
    if (products.length === 0) {
        ...
        productGrid.innerHTML = '<p style="color:red;">Không tìm thấy sản phẩm nào!</p>';
        return;
    }

    console.log("Selected category: ", category, "Search term: ", searchTerm);

    for (var i = 0; i < products.length; i++) {
        var productCategoryId = parseInt(products[i].getAttribute("data-category")) || 0;
        var productName = (products[i].querySelector(".product-name") || {}).textContent?.toLowerCase() || "";

        var matchesCategory = (category === "-1") || (productCategoryId === getCategoryIdFromName(category));
        var matchesSearch = (searchTerm === "") || productName.includes(searchTerm);

        products[i].style.display = (matchesCategory && matchesSearch) ? "block" : "none";
    }
}
...
function getCategoryIdFromName(categoryName) {
    const categoryMap = {
        "-1": 0, // All Categories
        "Ca_Nuoc_Lo": 8, // Cá nước lợ
        "Ca_Nuoc_Man": 9, // Cá nước mặn
        "Ca_Nuoc_Ngot": 10, // Cá nước ngọt
        "Thiet_bi": 7 // Trang thiết bị
    };
...
window.onload = function () {
    ...
    if (categorySelect && searchInput) {
        // Áp dụng trạng thái ban đầu từ param
        var initialCategory = "${param.category != null ? param.category : '-1'}";
        var initialSearch = "${param.s != null ? param.s : ''}";
        categorySelect.value = initialCategory;
        searchInput.value = initialSearch;
        filterProducts(); // Lọc ban đầu

        categorySelect.addEventListener("change", filterProducts);
        searchInput.addEventListener("input", filterProducts);
    } else {
        console.error("categoryFilter or searchInput not found on page load!");
    }
};
</script>
