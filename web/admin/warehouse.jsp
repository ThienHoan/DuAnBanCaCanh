...
<html>
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane fade ${activeTab eq 'products' or empty activeTab ? 'show active' : ''}" id="products">
                            <div class="table-container">
                                <!-- Show import button when form is hidden (tab products) -->
                                <c:if test="${param.showImport ne 'true' && (activeTab eq 'products' or empty activeTab)}">
                                    <div class="mb-4 text-end">
                                        <a href="${pageContext.request.contextPath}/admin/warehouse?tab=products&showImport=true" class="btn btn-primary">
                                            <i class="fas fa-file-import"></i> Nhập Dữ Liệu Sản Phẩm
                                        </a>
                                    </div>
                                </c:if>
                                <!-- Show import form only when showImport=true (tab products) -->
                                <c:if test="${param.showImport eq 'true' && (activeTab eq 'products' or empty activeTab)}">
                                    <div class="import-form mb-4">
                                        <h5><i class="fas fa-file-import"></i> Nhập Dữ Liệu Sản Phẩm (CSV)</h5>
                                        <form action="${pageContext.request.contextPath}/admin/warehouse" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
                                            <input type="hidden" name="action" value="import_product_csv">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <div class="file-upload-wrapper">
                                                        <label for="productCsvUpload" class="form-label">Chọn file CSV sản phẩm</label>
                                                        <input type="file" class="form-control" id="productCsvUpload" name="csvFile" accept=".csv" required>
                                                        <div class="help-text">
                                                            <i class="fas fa-info-circle"></i> Định dạng hỗ trợ: <strong>CSV</strong>.<br>
                                                            <strong>Cấu trúc file (thứ tự cột):</strong><br>
                                                            <code>category_id, name, description, short_description, price, sale_price, quantity, sku, status, featured, is_deleted</code>
                                                        </div>
                                                    </div>
                                                    <div class="form-text text-muted">
                                                        <i class="fas fa-exclamation-triangle text-warning"></i> <strong>Lưu ý:</strong> File CSV phải đúng định dạng với đầy đủ các cột như mẫu.<br>
                                                        <a href="${pageContext.request.contextPath}/ExportWarehouseServlet?type=product_template" class="text-primary" download><i class="fas fa-download"></i> <strong>Tải mẫu CSV</strong></a>
                                                    </div>
                                                </div>
                                        ...
                                        <!-- Success message -->
                                        <c:if test="${param.success eq 'import' && param.tab eq 'products'}">
                                            <div class="alert alert-success alert-dismissible fade show">
                                        ...
                                        <!-- Error message -->
                                        <c:if test="${param.error eq 'import' && param.tab eq 'products'}">
                                            <div class="alert alert-danger alert-dismissible fade show">
                                                <i class="fas fa-exclamation-circle"></i> Có lỗi xảy ra khi nhập sản phẩm!
                                                <c:if test="${not empty param.msg}">
                                                    <div class="mt-2 small">Chi tiết: ${param.msg}</div>
                                                </c:if>
                                                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                                        ...
                                        <c:if test="${not empty param.errorDetail}">
                                            <div class="alert alert-danger mt-3">
                                                <strong>Lỗi chi tiết khi nhập sản phẩm:</strong>
                                                <div>${fn:replace(param.errorDetail, '<br>', '<br/>')}</div>
                                            </div>
                                        </c:if>
                                    </div>
                                </c:if>
                                <!-- Thông báo cho tab sản phẩm -->
                                <c:if test="${(activeTab eq 'products' or empty activeTab) && (param.success eq 'import' || param.error eq 'import')}">
                                    <div class="mb-3">
                                ...
                                </c:if>                        <c:if test="${not empty listProduct}">
                        ...
                        <div class="tab-pane fade ${activeTab eq 'inventory' ? 'show active' : ''}" id="inventory">
                            <div class="table-container">
                                        ...
                                        <form action="${pageContext.request.contextPath}/ExportWarehouseServlet" method="post" enctype="multipart/form-data" accept-charset="UTF-8">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <div class="form-text text-muted">
                                                        <i class="fas fa-exclamation-triangle text-warning"></i> <strong>Lưu ý quan trọng:</strong> File Excel phải đúng định dạng với đầy đủ các cột như mẫu.
                                                        <a href="${pageContext.request.contextPath}/ExportWarehouseServlet?type=template&tab=inventory" class="text-primary">
                                                            <i class="fas fa-download"></i> <strong>Tải mẫu Excel để điền</strong>
                                                        </a>
                                                    </div>
                                                </div>
                                                <div class="col-md-6 d-flex align-items-end">
                                ...
                                <!-- Thông báo cho tab nhật ký kho hàng -->
                                <c:if test="${activeTab eq 'inventory' && (param.success eq 'import' || param.error eq 'import')}">
                                    <div class="mb-3">
                                        <c:if test="${param.success eq 'import'}">
                                            <div class="alert alert-success alert-dismissible fade show">
                                                <i class="fas fa-check-circle"></i> ${param.msg}
                                        ...
                                        <c:if test="${param.error eq 'import'}">
                                            <div class="alert alert-danger alert-dismissible fade show">
                                                <i class="fas fa-exclamation-circle"></i> Có lỗi xảy ra khi nhập kho hàng!
                                                <c:if test="${not empty param.msg}">
                                                    <div class="mt-2 small">Chi tiết: ${param.msg}</div>
                                                </c:if>
                                                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
                                        ...
                                        <c:if test="${not empty param.errorDetail}">
                                            <div class="alert alert-danger mt-3">
                                                <strong>Lỗi chi tiết khi nhập kho hàng:</strong>
                                                <div>${fn:replace(param.errorDetail, '<br>', '<br/>')}</div>
                                            </div>
                                        </c:if>
                                    </div>
    ...
    const headers = document.querySelectorAll('.sortable-header');
            ...
            if (urlParams.get('success') === 'import' || urlParams.get('error') === 'import' || urlParams.get('warning') === 'import') {
